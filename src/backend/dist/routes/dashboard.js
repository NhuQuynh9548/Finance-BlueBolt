...
const getTransactionAmountForBU = (transaction, buId, buName) => {
    ...
    if (transaction.costAllocation === 'INDIRECT' && transaction.allocationPreviews) {
        ...
        if (buId && buName) {
            const allocation = transaction.allocationPreviews.find((ap) => ap.buName === buName);
...
router.get('/stats', async (req, res) => {
    try {
        ...
        let targetBuId = undefined;
        let targetBuName = undefined;
        ...
        if (targetBuId) {
            const bu = await prisma_1.default.businessUnit.findUnique({ where: { id: targetBuId } });
            if (bu)
...
router.get('/bu-stats', async (req, res) => {
    try {
        const user = req.user;
        ...
        if (roleName === 'ceo' || roleName === 'admin') {
            if (buId && typeof buId === 'string' && buId !== 'all') {
                bus = await prisma_1.default.businessUnit.findMany({ where: { id: buId } });
            }
            else {
                bus = await prisma_1.default.businessUnit.findMany();
            }
        }
        else if (user.buId) {
            bus = await prisma_1.default.businessUnit.findMany({ where: { id: user.buId } });
        }
        else {
