...
router.post('/', async (req, res) => {
    try {
        ...
        // Find IDs from names
        const bu = await prisma_1.default.businessUnit.findFirst({ where: { name: businessUnit } });
        if (!bu) {
            console.error(`Business Unit not found: ${businessUnit}`);
            return res.status(400).json({ error: `Business Unit '${businessUnit}' not found` });
        }
        const spec = specialization ? await prisma_1.default.specialization.findFirst({ where: { name: specialization } }) : null;
        const lvl = level ? await prisma_1.default.employeeLevel.findFirst({ where: { name: level } }) : null;
...
router.put('/:id', async (req, res) => {
    try {
        ...
        let bu = null;
        if (businessUnit) {
            bu = await prisma_1.default.businessUnit.findFirst({ where: { name: businessUnit } });
            if (!bu) {
                return res.status(400).json({ error: `Business Unit '${businessUnit}' not found` });
            }
        }
